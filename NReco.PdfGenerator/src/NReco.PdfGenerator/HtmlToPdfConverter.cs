...
namespace NReco.PdfGenerator
{
    ...
    public class HtmlToPdfConverter
    {
		...
		public HtmlToPdfConverter() {
			...
			string rootDir = null;
#else
			string rootDir = Path.Combine( AppDomain.CurrentDomain.BaseDirectory, "wkhtmltopdf" );
#endif

#if !LIGHT 
			if (HttpContext.Current != null)
				rootDir = Path.Combine( HttpRuntime.AppDomainAppPath, @"App_Data\wkhtmltopdf" );
#endif
...
";
		...
		public void GeneratePdfFromFile(string htmlFilePath, string coverHtml, string outputPdfFilePath) {
			...
			GeneratePdfInternal( new [] { new WkHtmlInput(htmlFilePath) }, null, coverHtml, outputPdfFilePath, null);
		}
		...
		WkHtmlInput[] GetWkHtmlInputFromFiles(string[] files) {
			var rs = new WkHtmlInput[files.Length];
			for (int i=0; i<rs.Length; i++)
				rs[i] = new WkHtmlInput(files[i]);
			return rs;
		}
		...
		/// Generate PDF into specified output file by several HTML documents (local files or URLs) 
		/// </summary>
		/// <param name="htmlFiles">list of <see cref="WkHtmlInput"/></param>
		/// <param name="coverHtml">first page HTML (optional, can be null)</param>
		/// <param name="outputPdfFilePath">path to output PDF file (if file already exists it will be removed before PDF generation)</param>
		...
		private class PdfSettings {
			...
			public WkHtmlInput[] InputFiles = null;
		...
		private void GeneratePdfInternal(WkHtmlInput[] htmlFiles, string inputContent, string coverHtml, string outputPdfFilePath, Stream outputStream) { 
			...
			// generate global header/footer files
			...
			// generate page-specific header/footer files
			if (pdfSettings.InputFiles!=null)
				foreach (var input in pdfSettings.InputFiles) {
					input.HeaderFilePath = !String.IsNullOrEmpty(input.PageHeaderHtml) ? 
						CreateTempFile( String.Format(headerFooterHtmlTpl, input.PageHeaderHtml ), tempPath, tmpFiles) : null;
					input.FooterFilePath = !String.IsNullOrEmpty(input.PageFooterHtml) ?
						CreateTempFile( String.Format(headerFooterHtmlTpl,input.PageFooterHtml ), tempPath, tmpFiles ) : null;
				}
			...
			try {
				...
				if (inputContent != null) {
					pdfSettings.InputFiles = new[] {
						new WkHtmlInput( CreateTempFile(inputContent, tempPath, tmpFiles) )
					};
				}
