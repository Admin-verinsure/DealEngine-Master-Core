...
{
    public class NZFSGCLUWModule2021 : IUnderwritingModule
    {
        public string Name { get; protected set; }

        public NZFSGCLUWModule2021()
        {
            Name = "NZFSG_CL_2021";
        }
        ...
        public bool Underwrite(User underwritingUser, ClientInformationSheet informationSheet, Product product, string reference)
        {
            ...
            IDictionary<string, decimal> rates = BuildRulesTable(agreement, "cltermlimit", "cltermexcess", "cltermpremium", "cltermmaxfeeincome");
                ...
                if (renewFromAgreement != null)
                {
                    ...
                    foreach (var renewendorsement in renewFromAgreement.ClientAgreementEndorsements)
                    {
                        ...
                        if (renewendorsement.DateDeleted == null && renewendorsement.Name != "Social Engineering Fraud Extension" && 
                            renewendorsement.Name != "Data Recovery and Business Interruption Exclusion (DRB)" && 
                            renewendorsement.Name != "Unencrypted Portable Media Exclusion (UPM)")
                        {
            ...
            if (agreement.ClientInformationSheet.IsChange && agreement.ClientInformationSheet.PreviousInformationSheet != null)
            {
                ...
                foreach (var term in PreviousAgreement.ClientAgreementTerms)
                {
                    if (term.Bound)
                    {
                        ...
                        termcltermoption.PremiumDiffer = (TermPremium - PreviousBoundPremium) * coverperiodindaysforchange / agreementperiodindays;
                        termcltermoption.PremiumPre = PreviousBoundPremium;
                        if (termcltermoption.TermLimit == term.TermLimit && termcltermoption.Excess == term.Excess)
                        {
                            termcltermoption.Bound = true;
                        }
                        if (termcltermoption.PremiumDiffer < 0)
                        {
